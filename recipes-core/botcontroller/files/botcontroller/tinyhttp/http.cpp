// MODIFIED FOR THE SWEEPERBOT PROJECT
...
bool HttpRequest::parse(std::shared_ptr<IClientStream> stream) {
    ...
    if (cl > 0) {
        ...
        while( soFar < cl ) {
            ssize_t read = stream->receive(tmp + soFar, cl - soFar);
        ...
        if (    (*this)["Content-Type"] == "application/json"
            ...
            std::string error;
            try {
                mContentJson = nlohmann::json::parse(mContent);
            }
            catch(...) {
                if (!error.empty())
...
void HttpServer::startListening(uint16_t port) {
    ...
    setsockopt(mSocket, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(int));
    setsockopt(mSocket, SOL_SOCKET, SO_REUSEPORT, &reuse, sizeof(int));
